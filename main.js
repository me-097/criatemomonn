window.onload = () => {
    ...
    const ROOMS = [
        // ── 1階 ──────────────────────────────────────────────
        {
            ...
            title:  'リンクスホール',
            detail: 'ステージ発表やライブパフォーマンスが行われます。',
            tags:   ['ステージ', 'ライブ', 'メイン会場'],
        },
        {
            name:   '保健室',
    ...
    rooms.forEach((room, idx) => {
        ...
        group.add(new THREE.Mesh(new THREE.SphereGeometry(19, 32, 32), glowMat));
        ...
        const sphere = new THREE.Mesh(new THREE.SphereGeometry(11, 32, 32), sphereMat);
        sphere.userData = { roomIndex: idx };
        group.add(sphere);

        // ピン
        const pinMat = new THREE.MeshStandardMaterial({ color: col, transparent: true, opacity: 0.65 });
        const pin = new THREE.Mesh(new THREE.CylinderGeometry(2, 2, 55, 8), pinMat);
        pin.position.y = -28;
        group.add(pin);

        // 先端
        const tipMat = new THREE.MeshStandardMaterial({ color: col });
        const tip = new THREE.Mesh(new THREE.ConeGeometry(4, 9, 8), tipMat);
        tip.position.y = -60; tip.rotation.x = Math.PI;
        group.add(tip);

        group.position.set(room.x, room.y + 72, room.z);
        group.userData = { roomIndex: idx, baseY: room.y + 72 };
        scene.add(group);
        markers.push(group);
